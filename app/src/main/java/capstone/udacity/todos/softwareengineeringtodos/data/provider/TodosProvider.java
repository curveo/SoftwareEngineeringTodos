...
import capstone.udacity.todos.softwareengineeringtodos.data.provider.TodosContent.Todos;
...
public final class TodosProvider extends ContentProvider {

    private static final String LOG_TAG = TodosProvider.class.getSimpleName();

    /* package */ static final boolean ACTIVATE_ALL_LOGS = false;

    protected static final String DATABASE_NAME = "TodosProvider.db";

    public static final String AUTHORITY = "capstone.udacity.todos.softwareengineeringtodos";
    ...
    private enum UriType {
        TODOS(Todos.TABLE_NAME, Todos.TABLE_NAME, Todos.TYPE_ELEM_TYPE),
        TODOS_ID(Todos.TABLE_NAME + "/#", Todos.TABLE_NAME, Todos.TYPE_DIR_TYPE);
    ...
    private class DatabaseHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
            Log.d(LOG_TAG, "Creating TodosProvider database");

            // Create all tables here; each class has its own method
            Todos.createTable(db);
        }
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            ...
            Todos.upgradeTable(db, oldVersion, newVersion);
        }
    ...
    public int bulkInsert(Uri uri, ContentValues[] values) {
        ...
        try {
            switch (uriType) {
                case TODOS:
                    insertStmt = db.compileStatement(Todos.getBulkInsertString());
                    for (ContentValues value : values) {
                        Todos.bindValuesInBulkInsert(insertStmt, value);
                        insertStmt.execute();
